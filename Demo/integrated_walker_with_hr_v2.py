	...
	while True:
		try:
			...
			if 'No finger?' in current_line:
				...
				with open('heartrate_raw.csv', 'a', newline='') as f:
					f.write(f'Timestamp={datetime.now()}, {current_line}\n')
					
				new_avg_bpm = float(current_line.split(",")[2].split("=")[1])
				if new_avg_bpm != current_avg_bpm:
					current_avg_bpm = new_avg_bpm

					# value is greater than 80 OR less than 60
					# If values is between 60 - 80
					# 	maintain speed 
					# otherwise
					# 	reduce speed
					if current_avg_bpm >= 75 or current_avg_bpm <= 55:
						stop_counter = stop_counter + 1

						if stop_counter > 3:
							if has_spoken is False:
								speak("stop_due_to_hr")
								has_spoken = True
							a=2
							m=3
							qw=3
					...
					with open('heartrate.csv','a',newline='') as f:
						f.write(f'Timestamp={datetime.now()}, Avg BPM={current_avg_bpm}, Stop Counter={stop_counter}\n')
...
def clicked():
	...
	while a==1:
		...
		if "start" in y:
			...
			speak("confirm_desired_distance")
			con="6"
			...
			wow_thread.start()
			hr_thread = threading.Thread(target=start_hr)
			hr_thread.start()
			# # Comment if no insole sensor
